...
class TextCNN(nn.Module):
    def __init__(self, featureSize, filterSize, contextSizeList, reduction='pool', actFunc=nn.ReLU, bn=False, name='textCNN'):
        ...
        self.batcnNormList = nn.ModuleList([nn.BatchNorm1d(filterSize) for i in range(len(contextSizeList))])
#         self.batcnNorm = nn.BatchNorm1d(filterSize)
    ...
    def forward(self, x):
        ...
        if self.bn:
            x = [self.batcnNormList[i](j.transpose(1,2)).transpose(1,2) for i,j in enumerate(x)]
#             x = [self.batcnNorm(j.transpose(1,2)).transpose(1,2) for i,j in enumerate(x)]
        x = [self.actFunc(i) for i in x] # => scaleNum * (batchSize × seqLen × filterSize)
...
class PreLN_SelfAttention_seq2seq(nn.Module):
    def __init__(self, maxItems, featureSize, dk, multiNum, name='selfAttn'):
        ...
        self.WQ = nn.Linear(featureSize, self.dk*multiNum)
        self.WK = nn.Linear(featureSize, self.dk*multiNum)
        self.WV = nn.Linear(featureSize, self.dk*multiNum)
    ...
    def forward(self, x, xlen=None):
        ...
        x = self.layerNorm1(x)
        B,L,C = x.shape      
        queries = self.WQ(x).reshape(B,L,self.multiNum,self.dk).transpose(1,2) # => batchSize × multiNum × seqLen × dk
        keys = self.WK(x).reshape(B,L,self.multiNum,self.dk).transpose(1,2) # => batchSize × multiNum × seqLen × dk
        values  = self.WV(x).reshape(B,L,self.multiNum,self.dk).transpose(1,2) # => batchSize × multiNum × seqLen × dk = batchSize * 8 * 153 * 48
        
        scores = queries@keys.transpose(-1,-2) / np.sqrt(self.dk) # => batchSize × multiNum × seqLen × seqLen
        mask = torch.zeros(scores[:,0].shape, dtype=torch.float32, device=scores.device) # => batchSize × seqLen × seqLen
        ...
        mask = mask.unsqueeze(dim=1) # => batchSize × 1 × seqLen × seqLen
        scores = F.softmax(scores+mask, dim=3) # =>batchSize × multiNum × seqLen × seqLen
        self.scores = scores

        z = scores @ values # => batchSize × multiNum × seqLen × dk
...
class TextPreLNTransformer_seq2seq(nn.Module):
    def __init__(self, seqMaxLen, layersNum, featureSize, dk, multiNum, maxItems=10, dropout=0.1, usePos=True, name='textTransformer'):
        ...
        posEmb = [[np.sin(pos/10000**(i/featureSize)) if i%2==0 else np.cos(pos/10000**((i-1)/featureSize)) for i in range(featureSize)] for pos in range(seqMaxLen)]
    ...
    def forward(self, x, xlen=None):
        ...
        x, _ = self.transformerLayers((x, xlen)) # => batchSize × seqLen × feaSize
        return x, _
