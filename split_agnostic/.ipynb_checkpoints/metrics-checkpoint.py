...
def remove_blank(labels, blank=0):
    ...
    for l in labels:
        if l != previous:
            ...
            keep_index.append(False)
    return new_labels,keep_index
    
class Metrictor:
    def __init__(self):
        self._reporter_ = {"ACC":self.ACC, "AUC":self.AUC, "LOSS":self.LOSS, "MSE":self.MSE, "MAE":self.MAE, 
                           "seqErrorDivSeq":self.seqErrorDivSeq, "editDistDivSeq":self.editDistDivSeq, "editDistDivSymbol":self.editDistDivSymbol}
    ...
    def fastEditDist(self):
        np.savez('tmp.npz', Y_pre=self.Y1_pre, Y=self.Y1)
        os.system(f'python multi_thread_edit_dist.py --n_jobs 8')
        data = np.load('tmp.npz')
        editDist = int(np.load('tmp.npz')['editDist'])
        return editDist
